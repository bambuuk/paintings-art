...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.parentNode.appendChild(statusMessage); // скрываем форму, чтобы вместо неё добавить изображение статуса отправки

      item.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        item.style.display = 'none';
      }, 400); // добавили спинер при отправки формы
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    var matrix = '+7 (___) ___ __ __',
        ...
        // убираем все нечисловые значения - статичное значение на основе матрицы
    val = this.value.replace(/\D/g, ''); // динамичное значение, работает на основе того, что ввёл пользователь
...
var modals = function modals() {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display;
      /** Если у нас открыта модалка - ничего не отображаем */
      ...
      /** Если никакая модалка невыведена - показываем нужную нам модалку  */
  ...
  /** Функция вызова подарка, когда пользователь упустился в самый низ страницы */
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 1; // текущий слайд, который показывается пользователю
  ...
  /** Если навели мышкой на слайдер, то прекращает автоматически пролистывать слайды */


  items[0].parentNode.addEventListener('mouseenter', function () {
    clearInterval(paused);
  });
  /** Если курсор убрали из слайдера, заново активируем автоматическое воспроизведение */
